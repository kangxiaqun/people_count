    ...
    def conv_layer(self, bottom, size, stride, in_channels, out_channels, use_bn, name):
        with tf.variable_scope(name):
            ...
            if use_bn:
                ...
                act = conv
        return act
    def build(self, feat_ex,res18, res10, res2):
        ...
        self.conv75 = self.conv_layer(self.conv72, 1, 1, 128, 64, True, 'conv_head_75')    # 52*64
        size = tf.shape(self.conv75)[1]
        self.upsample2 = tf.image.resize_nearest_neighbor(self.conv75, [2 * size, 2 * size],
                                                          name='upsample_2') 
        self.route2 = tf.concat([self.upsample2, res2], axis=-1, name='route_2')           # 104*192
        self.conv76 = self.conv_layer(self.route2, 1, 1, 192, 64, True, 'conv_head_76')    # 104*64
        self.conv77 = self.conv_layer(self.conv76, 3, 1, 64, 128, True, 'conv_head_77')    # 104*128
        self.conv78 = self.conv_layer(self.conv77, 1, 1, 128, 64, True, 'conv_head_78')    # 104*64
        self.conv79 = self.conv_layer(self.conv78, 3, 1, 64, 128, True, 'conv_head_79')    # 104*128
        self.conv80 = self.conv_layer(self.conv79, 1, 1, 128, 64, True, 'conv_head_80')    # 104*64
        self.conv81 = self.conv_layer(self.conv80, 3, 1, 64, 128, True, 'conv_head_81')    # 104*128
        self.conv82 = self.conv_layer(self.conv81, 1, 1, 128, 15, False, 'conv_head_82')    # 104*15
...
class yolo_det:
    ...
    def build(self, feats):
        ...
        # ...
        # 12, 1
        conv_index = tf.reshape(conv_index, [1, conv_dims[1], conv_dims[0], 1, 2])  # [1, 104, 104, 1, 2]
        ...
        box_xy = tf.sigmoid(feats[..., :2])  # σ(tx), σ(ty)     # [None, 104, 104, 3, 2] 
        box_twh = feats[..., 2:4] ## w,h
        box_wh = tf.exp(box_twh)  # exp(tw), exp(th)    # [None, 104, 104, 3, 2]
        ...
        # box_xy: [None, 104, 104, 3, 2]
        # box_wh: [None, 104, 104, 3, 2]
        # box_confidence: [None, 104, 104, 3, 1]
...
def preprocess_true_boxes(true_boxes, anchors, feat_size, image_size): 
    ...
    true_maxes = true_wh_half ##尺寸：[batch, 30, 1, 2],值：[-wh/2,wh/2]
    ...
    intersect_areas = intersect_wh[..., 0] * intersect_wh[..., 1]   # [batch, 30, num_anchors] 每个anchor与原图img的相交面积

    true_areas = true_wh[..., 0] * true_wh[..., 1]      # [batch, 30, 1]  ##每个box的面积
    ...
    iou_scores = intersect_areas / union_areas  # [batch, 30, num_anchors] ##计算每个box与每个anchor的IOU
    ...
    idx_one_hot = tf.reshape(idx_one_hot,
                        [-1, cfg.max_truth, feat_size[1], feat_size[2], num_anchors,
                         1])  # (batch, 30, 104, 104, 3, 1)
    loc_scale = 2 - true_boxes[..., 2] * true_boxes[..., 3]     # ???意义是什么？？(batch, 30)
    mask = []
    loc_cls = [] 
    scale = []
    for i in range(cfg.batch_size):
        idx_i = tf.where(valid[i])[:, 0]    # (?, )    # false / true
        mask_i = tf.gather(idx_one_hot[i], idx_i)   # (?, 104, 104, 3, 1)

        scale_i = tf.gather(loc_scale[i], idx_i)    # (?, )
        scale_i = tf.reshape(scale_i, [-1, 1, 1, 1, 1])     # (?, 1, 1, 1, 1)
        scale_i = scale_i * mask_i      # (?, 104, 104, 3, 1)
        scale_i = tf.reduce_sum(scale_i, axis=0)        # (104, 104, 3, 1)
        ...
        true_boxes_i = tf.gather(true_boxes[i], idx_i)    # (?, 4)
        sig_xy_i = tf.gather(sig_xy[i], idx_i)    # (?, 2)
        anchors_cf_i = tf.gather(anchors_cf[i], idx_i)    # (?, 2)
        twh = tf.log(true_boxes_i[:, 2:4] / anchors_cf_i)
        loc_cls_i = tf.concat([sig_xy_i, twh], axis=-1)    # (?, 4)
        loc_cls_i = tf.reshape(loc_cls_i, [-1, 1, 1, 1, 4])     # (?, 1, 1, 1, 5)
        loc_cls_i = loc_cls_i * mask_i      # (?, 104, 104, 3, 5)
        loc_cls_i = tf.reduce_sum(loc_cls_i, axis=[0])  # (104, 104, 3, 5)
        loc_cls.append(loc_cls_i)

        mask_i = tf.reduce_sum(mask_i, axis=[0])    # (104, 104, 3, 1)
        mask_i = tf.minimum(mask_i, 1)
        mask.append(mask_i)
    loc_cls = tf.stack(loc_cls, axis=0)     # (σ(tx), σ(tx), tw, th)
...
def box_IoU(b1, b2):
    ...
    with tf.name_scope('BB1'):
        ...
        b1 = tf.expand_dims(b1, -2)  # shape= (None, 104, 104, 3, 1, 4)
        b1_xy = b1[..., :2]  # x,y shape=(None,104, 104, 3, 1, 2)
        b1_wh = b1[..., 2:4]  # w,h shape=(None, 104, 104, 3, 1, 2)
        b1_wh_half = b1_wh / 2.  # w/2, h/2 shape= (None, 104, 104, 3, 1, 2)
        b1_mins = b1_xy - b1_wh_half  # x,y: left bottom corner of BB
        b1_maxes = b1_xy + b1_wh_half  # x,y: right top corner of BB
        b1_area = b1_wh[..., 0] * b1_wh[..., 1]  # w1 * h1 (None,104, 104, 3, 1)

    with tf.name_scope('BB2'):
        """Calculate 2 corners: {left bottom, right top} and area of this box"""
        b2 = tf.expand_dims(b2, -2)  # shape= (None, 104, 104, 3, 1, 4)
        #b2 = tf.expand_dims(b2, 0)  # shape= (1, None, 104, 104, 3, 4)  # TODO 0?
        b2_xy = b2[..., :2]  # x,y shape=(None,104, 104, 3, 1, 2)
        b2_wh = b2[..., 2:4]  # w,h shape=(None, 104, 104, 3, 1, 2)
        b2_wh_half = b2_wh / 2.  # w/2, h/2 shape=(None, 104, 104, 3, 1, 2)
    ...
    with tf.name_scope('Intersection'):
        ...
        # intersect_mins = tf.maximum(b1_mins, b2_mins, name='left_bottom')  # (None, 104, 104, 3, 1, 2)
        ...
        # intersect_wh = tf.maximum(intersect_maxes - intersect_mins, 0.)  # (None, 104, 104, 3, 1, 2), 2: w,h
        intersect_wh = tf.maximum(intersect_maxes - intersect_mins, 0.)
        intersect_area = intersect_wh[..., 0] * intersect_wh[..., 1]  # intersection: wi * hi (None, 104, 104, 3, 1)

    IoU = tf.divide(intersect_area, (b1_area + b2_area - intersect_area), name='divise-IoU')  # (None, 104, 104, 3, 1)
...
def confidence_loss(pred_xy, pred_wh, pred_confidence, true_boxes, detectors_mask):
    ...
    :return:
    """
    pred_xy = tf.expand_dims(pred_xy, 4)  # [batch, 104, 104, 3, 1, 2]
    pred_wh = tf.expand_dims(pred_wh, 4)  # [batch, 104, 104, 3, 1, 2]
    ...
    # [batch,104, 104, 3, 1, 2] [batch, 1, 1, 1, num_true_boxes, 2]
    intersect_mins = tf.maximum(pred_mins, true_mins)
    # [batch,104, 104, 3, num_true_boxes, 2]
    intersect_maxes = tf.minimum(pred_maxes, true_maxes)
    # [batch, 104, 104, 3, num_true_boxes, 2]
    intersect_wh = tf.maximum(intersect_maxes - intersect_mins, 0.)
    # [batch, 104, 104, 3, num_true_boxes, 2]
    intersect_areas = intersect_wh[..., 0] * intersect_wh[..., 1]
    # [batch, 104, 104, 3, num_true_boxes]
